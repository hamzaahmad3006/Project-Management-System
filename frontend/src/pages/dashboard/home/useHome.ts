...
export const useHomeHook = () => {
  ...
  const completionPercentage = useMemo(() => {
    return kpis?.tasks?.total ? Math.round((kpis.tasks.completed / kpis.tasks.total) * 100) : 0;
  }, [kpis]);
        ...
        tooltip: {
          ...
          callbacks: {
            ...
            title: (items) => {
              ...
              const month =
                dataPoint?.month || new Date().toLocaleDateString('en-US', { month: 'short' });
  ...
  const latestTasks = useMemo(
    () => recentActivity.slice(0, 4) as DashboardTask[],
    [recentActivity]
  );
