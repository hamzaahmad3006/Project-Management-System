  ...
  const columns = [
    {
      ...
      title: 'To-Do',
      color: 'bg-blue-500',
      items: filteredTasks.filter((t: Task) => t.status === 'TODO' || !t.status),
    },
    {
      id: 'IN_PROGRESS',
      title: 'In Progress',
      color: 'bg-orange-500',
      items: filteredTasks.filter((t: Task) => t.status === 'IN_PROGRESS'),
    },
    {
      id: 'COMPLETED',
      title: 'Completed',
      color: 'bg-green-500',
      items: filteredTasks.filter((t: Task) => t.status === 'COMPLETED'),
    },
    {
      id: 'CANCELED',
      title: 'Cancelled',
      color: 'bg-red-500',
      items: filteredTasks.filter((t: Task) => t.status === 'CANCELED'),
    },
  ];
