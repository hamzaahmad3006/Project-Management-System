...
export default function DashboardPage() {
    const { loading, displayStats, chartData, chartOptions, hours, todayEvents, currentDateDisplay, filteredTeamProjects, setSelectedProjectId, selectedProjectId, selectedYear, setSelectedYear, dispatch
    } = useDashboardTabHook()
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white dark:bg-[#1a1c23] border border-gray-100 dark:border-gray-800 rounded-xl overflow-hidden shadow-sm flex flex-col">
                        <div className="p-6 pb-2 border-b border-gray-50 dark:border-gray-800/50 flex justify-between items-center">
                            ...
                            <div className="text-xs font-bold text-gray-400 dark:text-gray-500">{currentDateDisplay}</div>
                        </div>
                                ...
                                <div className="flex bg-gray-50/50 dark:bg-gray-800/30 border-b border-gray-100 dark:border-gray-800/50">
                                    {hours.map(h => (
                                        <div key={h} className="flex-1 py-3 text-center border-r border-gray-100/50 dark:border-gray-800/20 last:border-0">
                                            <span className="text-[10px] font-bold text-gray-400 dark:text-gray-600 uppercase tracking-tighter">{h}</span>
                                        </div>
                                    ))}
                                </div>
                                    ...
                                    {todayEvents.length > 0 ? (
                                        todayEvents.slice(0, 4).map((event: CalendarEvent, idx: number) => {
                                            ...
                                            const typeColors: Record<string, string> = {
                                                ...
                                                'EVENT': 'bg-blue-50/70 border-blue-400 text-blue-900 dark:bg-blue-900/20 dark:text-blue-300 dark:border-blue-700'
                                            };
                                        ...
                                        <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 gap-2">
                                            <div className="w-12 h-12 rounded-full bg-gray-50 dark:bg-gray-800/50 flex items-center justify-center mb-1">📅</div>
